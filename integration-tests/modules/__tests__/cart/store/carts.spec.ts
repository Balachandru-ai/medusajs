  ...
  testSuite: ({ dbConnection, getContainer, api }) => {
    describe("Store Carts API", () => {
      ...
      describe("POST /store/carts/:id/complete", () => {
        ...
        it.only("should not delete orders if payment authorization fails", async () => {
          const firstCart = (
            await api.post(
              `/store/carts`,
              {
          ...
          await api.post(
            `/store/payment-collections/${firstPaymentCollection.id}/payment-sessions`,
            { provider_id: "pp_system_default" },
            storeHeaders
          )

          const firstOrderResponse = await api.post(
            `/store/carts/${firstCart.id}/complete`,
            {},
            storeHeaders
          )

          expect(firstOrderResponse.status).toEqual(200)
          expect(firstOrderResponse.data).toEqual({
            type: "order",
            order: expect.objectContaining({
              id: expect.any(String),
            }),
          ...
          await api.post(
            `/store/payment-collections/${secondPaymentCollection.id}/payment-sessions`,
            { provider_id: "pp_system_default" },
            storeHeaders
          )

          const secondOrderResponse = await api.post(
            `/store/carts/${secondCart.id}/complete`,
            {},
            storeHeaders
          )

          expect(secondOrderResponse.status).toEqual(200)
          expect(secondOrderResponse.data).toEqual({
            type: "order",
            order: expect.objectContaining({
              id: expect.any(String),
            }),
          ...
          // Mock the payment authorization to fail
          ...
          expect(orders.data.orders.length).toEqual(2)
          expect(orders.data.orders).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
                id: firstOrderResponse.data.order.id,
              }),
              expect.objectContaining({
                id: secondOrderResponse.data.order.id,
              }),
            ])
          )
        })
