...
export class Migration20260106185528 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_order_line_item_adjustment_version_item" ON "order_line_item_adjustment" ("version", "item_id") WHERE deleted_at IS NULL;`
    )
  ...
  override async down(): Promise<void> {
    this.addSql(
      `drop index if exists "IDX_order_line_item_adjustment_version_item";`
    )
