...
export class Migration20241122120331 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'alter table if exists "image" add column if not exists "rank" integer not null default 0, add column if not exists "product_id" text null;'
    )
    ...
    this.addSql(
      'alter table if exists "image" alter column "product_id" set not null;'
    )
    this.addSql(
      'alter table if exists "image" add constraint "image_product_id_foreign" foreign key ("product_id") references "product" ("id") on update cascade on delete cascade;'
    )
  ...
  async down(): Promise<void> {
    ...
    this.addSql(
      'alter table if exists "image" drop constraint if exists "image_product_id_foreign";'
    )
    this.addSql('alter table if exists "image" drop column if exists "rank";')
    this.addSql(
      'alter table if exists "image" drop column if exists "product_id";'
    )
  }
}
