...
export class Migration20250929204436 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table if not exists "product_variant_product_image" ("id" text not null, "created_by" text null, "metadata" jsonb null, "variant_id" text not null, "image_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "product_variant_product_image_pkey" primary key ("id"));`
    )
    this.addSql(
