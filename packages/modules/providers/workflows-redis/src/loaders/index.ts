...
export default async (
  ...
  if (url && !redisUrl) {
    logger?.warn(
      "[Workflows-redis] The `url` option is deprecated. Please use `redisUrl` instead for consistency with other modules."
    )
  }
  if (deprecatedRedisOptions && !newRedisOptions) {
    logger?.warn(
      "[Workflows-redis] The `options` option is deprecated. Please use `redisOptions` instead for consistency with other modules."
    )
  }
  ...
  if (!resolvedUrl) {
    throw Error(
      "No `redisUrl` (or deprecated `url`) provided in workflows-redis provider options. It is required for the Workflows Redis provider."
    )
  }
  ...
  try {
    ...
    logger?.info(
      `[Workflows-redis] Connection to Redis in provider 'workflows-redis' established`
    )
  } catch (err) {
    logger?.error(
      `[Workflows-redis] An error occurred while connecting to Redis in provider 'workflows-redis': ${err}`
    )
  }
  ...
  try {
    ...
    logger?.info(
      `[Workflows-redis] Connection to Redis PubSub in provider 'workflows-redis' established`
    )
  } catch (err) {
    logger?.error(
      `[Workflows-redis] An error occurred while connecting to Redis PubSub in provider 'workflows-redis': ${err}`
    )
  }
  ...
  container.register({
    ...
    workflowsStorage: asClass(RedisWorkflowsStorage).singleton(),
    workflowsProviderDisconnectHandler: asValue(async () => {
