...
import { IStoreCreditModuleService, PluginModule } from "../../../../src/types";
...
medusaIntegrationTestRunner({
  testSuite: ({ api, getContainer }) => {
    ...
    beforeEach(async () => {
      ...
      const user2 = await createStoreUser({
        api,
        storeHeaders,
      ...
      storeHeaders2.headers["Authorization"] = `Bearer ${user2.token}`;
    ...
    describe("POST /store/store-credit-accounts/claim", () => {
      it("should claim an anonymous account for a customer", async () => {
        const container = getContainer();
        const storeCreditService = container.resolve<IStoreCreditModuleService>(
