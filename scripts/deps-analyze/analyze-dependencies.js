...
const packagesOutput = execSync('find packages -name "package.json" -not -path "*/node_modules/*"', {
...
sortedDeps.forEach(([dep, info]) => {
  const versionInfo = info.versions.size > 1 ? ` ⚠️  (${info.versions.size} versions)` : '';
...
if (fs.existsSync(nodeModulesPath)) {
  try {
    ...
    const duOutput = execSync(`du -sh node_modules/* 2>/dev/null | sort -hr | head -50`, {
      cwd: path.join(__dirname, '..'),
      encoding: 'utf8',
