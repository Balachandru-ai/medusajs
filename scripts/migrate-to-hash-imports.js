...
function findModules() {
  ...
  for (const entry of entries) {
    ...
    // Check if module has path aliases OR hash imports
    const tsconfig = JSON.parse(fs.readFileSync(tsconfigPath, "utf8"))
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"))
    const hasPaths = tsconfig.compilerOptions?.paths
    const hasHashImports =
      packageJson.imports && Object.keys(packageJson.imports).length > 0

    if (hasPaths || hasHashImports) {
      modules.push({
        name: moduleName,
        path: modulePath,
        tsconfigPath,
...
function convertBarrelImports(content, aliases, exportMaps) {
  ...
  for (const [key, info] of Object.entries(aliases)) {
    ...
    const multiImportPattern = new RegExp(
      `import\\s*{([^}]+)}\\s*from\\s+['"]${original.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&"
      )}['"]`,
      "g"
    ...
    modified = modified.replace(multiImportPattern, (match, imports) => {
      ...
      changes.push(
        `${original} (multi-import) -> ${hashImport}/* (individual imports)`
      )
...
/**
 * Extract hash import patterns from package.json imports field
 * Returns a map of directory name -> hash import prefix
 * Example: { "services": "#services", "types": "#types" }
 */
function extractHashImportsFromPackageJson(packageJsonPath) {
  const hashImports = {}
  ...
  // Parse patterns like "#services/*": "./dist/services/*.js"
  for (const [key, value] of Object.entries(packageJson.imports)) {
    ...
    if (keyMatch && valueMatch) {
      const hashPrefix = keyMatch[0].replace(/\/\*$/, "") // "#services"
      const dirName = valueMatch[1] // "services"
      hashImports[dirName] = hashPrefix
    }
  }
...
/**
 * Convert relative imports to hash imports
 * Example: from "../services/file" -> from "#services/file"
 */
function convertRelativeImports(content, hashImports) {
  let modified = content
  const changes = []

  for (const [dirName, hashPrefix] of Object.entries(hashImports)) {
    // Pattern to match relative imports like:
    // from "../services/file"
    // from "../../services/file"
    // Works with any number of ../ traversals
    ...
    modified = modified.replace(
      relativePattern,
      (match, quote1, dots, filePath, quote2) => {
        return `from ${quote1}${hashPrefix}/${filePath}${quote2}`
      }
    )

    if (modified !== oldContent) {
      changes.push(`../${dirName}/* -> ${hashPrefix}/*`)
    }
  }
...
/**
 * Convert namespace imports from barrel files in mikro-orm.config.dev.ts
 * Example: import * as entities from "./src/models" -> individual imports
 */
function convertMikroOrmConfigImports(content, modulePath, hashImports) {
  let modified = content
  const changes = []

  // Pattern to match: import * as entities from "./src/models"
  const namespaceImportPattern =
    /import\s+\*\s+as\s+(\w+)\s+from\s+['"](\.\/src\/models)['"]/

  const match = content.match(namespaceImportPattern)
  if (!match) {
    return { content, changes: [], modified: false }
  }

  const [fullMatch, namespace, importPath] = match

  // Parse the models barrel file to get all exported model names
  const modelsBarrelPath = path.join(modulePath, "src", "models", "index.ts")
  const exportMap = parseBarrelFile(modelsBarrelPath)

  if (Object.keys(exportMap).length === 0) {
    // No exports found, skip
    return { content, changes: [], modified: false }
  }

  const modelNames = Object.keys(exportMap)

  // Generate individual imports
  let individualImports
  const useHashImports = hashImports.models

  if (useHashImports) {
    // Use hash imports: import ApiKey from "#models/api-key"
    individualImports = modelNames
      .map((name) => {
        const sourceFile = exportMap[name]
        return `import ${name} from "${useHashImports}/${sourceFile}"`
      })
      .join("\n")
    changes.push(`${importPath} namespace import -> ${useHashImports}/* (individual imports)`)
  } else {
    // Use relative imports: import ApiKey from "./src/models/api-key"
    individualImports = modelNames
      .map((name) => {
        const sourceFile = exportMap[name]
        return `import ${name} from "${importPath}/${sourceFile}"`
      })
      .join("\n")
    changes.push(`${importPath} namespace import -> ${importPath}/* (individual imports)`)
  }

  // Replace the namespace import
  modified = modified.replace(fullMatch, individualImports)

  // Replace Object.values(namespace) with [Model1, Model2, ...]
  ...
  return { content: modified, changes, modified: true }
}
...
function findAllSourceFiles(modulePath) {
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (
          !["__tests__", "__mocks__", "node_modules", "dist"].includes(
            entry.name
          )
        ) {
          walk(fullPath)
  ...
  // Also include mikro-orm.config.dev.ts if it exists
  const mikroOrmConfigPath = path.join(modulePath, "mikro-orm.config.dev.ts")
  if (fs.existsSync(mikroOrmConfigPath)) {
    files.push(mikroOrmConfigPath)
  }
...
function updateImportsInFile(
  ...
  // Check if this is a mikro-orm.config.dev.ts file
  const isMikroOrmConfig = path.basename(filePath) === "mikro-orm.config.dev.ts"

  if (isMikroOrmConfig) {
    // Handle mikro-orm config files specially
    const mikroOrmResult = convertMikroOrmConfigImports(
      content,
      modulePath,
      hashImports
    )
    content = mikroOrmResult.content
    allChanges.push(...mikroOrmResult.changes)

    const modified = mikroOrmResult.modified
  ...
  // For regular files, apply all transformations
  ...
  // Third: Convert relative imports to hash imports
  const relativeResult = convertRelativeImports(content, hashImports)
  content = relativeResult.content
  allChanges.push(...relativeResult.changes)

  const modified =
    barrelResult.modified || simpleResult.modified || relativeResult.modified
...
function processModule(module, dryRun, backup, keepBarrels) {
  ...
  log(
    `\nFound aliases: ${Object.keys(aliases)
      .map((k) => `@${k}`)
      .join(", ")}`,
    "blue"
  )
  ...
  // Extract hash imports from package.json for relative import conversion
  const hashImports = extractHashImportsFromPackageJson(module.packageJsonPath)
  if (Object.keys(hashImports).length > 0) {
    log("\nFound hash imports in package.json:", "blue")
    for (const [dirName, hashPrefix] of Object.entries(hashImports)) {
      log(`  ${dirName} -> ${hashPrefix}`, "blue")
    }
  }
  ...
  log(
    `  Found ${sourceFiles.length} TypeScript files (src/ + integration-tests/)`,
    "blue"
  )
  ...
  if (filesModified > 0) {
    ...
    if (allChanges.length <= 10) {
      ...
      for (let i = 0; i < 10; i++) {
        for (const { file } of allChanges[i]) {
          log(`    ${file}`, "blue")
        }
      }
...
async function main() {
  ...
  if (options.module) {
    ...
    if (modules.length === 0) {
      log(
        `\n❌ Module '${options.module}' not found or has no path aliases`,
        "red"
      )
      process.exit(1)
    }
