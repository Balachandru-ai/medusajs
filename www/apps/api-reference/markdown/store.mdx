...
- Use the JS SDK's `setLocale` method. The JS SDK will automatically include the locale in the `Content-Language` header of subsequent requests.
- Pass the `locale` query parameter to the [List Products API route](#products_getproducts).
- Set the `Content-Language` header in the API request to the [List Products API route](#products_getproducts).
