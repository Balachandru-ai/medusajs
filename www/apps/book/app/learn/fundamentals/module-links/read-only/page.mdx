...
### Relation Name for Both Link Types

Whether you're using a one-to-one or one-to-many relation for read-only module links, the relation name is always the alias of the second data model. It's not changed to its plural form for one-to-many relations.
