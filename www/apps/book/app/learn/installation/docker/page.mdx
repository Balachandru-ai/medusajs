...
**For Windows Users:** Ensure that the `start.sh` file uses LF line endings instead of CRLF. Git on Windows can sometimes automatically convert line endings, which causes errors when running the script inside the Linux-based Docker container. You can configure Git or your code editor to maintain LF line endings to prevent this issue.
