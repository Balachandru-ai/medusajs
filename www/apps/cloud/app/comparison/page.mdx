...
- **Framework agnostic**: Deploy storefronts built with any frontend framework, such as Tanstack Start, Next.js, or Vite.
...
- **Multi-storefront support**: Deploy multiple storefronts (for example, DTC and B2B) from the same monorepo, all connected to the same Medusa server.
