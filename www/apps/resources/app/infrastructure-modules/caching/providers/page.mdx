...
For development, you can use the [In-Memory Caching Module Provider](./in-memory/page.mdx). This provider stores cached data in the memory of your Medusa application, which is suitable for development and testing purposes.

However, for production, it's recommended to use other Caching Module Providers, such as the [Redis Caching Module Provider](./redis/page.mdx). You can also [Create a Caching Provider](/references/caching-module-provider).
...
If you don't configure the Caching Module in `medusa-config.ts` and you enable the [In-Memory Caching Module Provider](./in-memory/page.mdx), Medusa uses it as the default provider.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        In-Memory Caching Module Provider if enabled.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
...
If none of the above scenarios apply, Medusa throws an error during startup indicating that no default provider is configured.
