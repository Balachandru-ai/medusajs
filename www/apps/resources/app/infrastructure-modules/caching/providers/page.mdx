...
By default, Medusa uses the [In-Memory Caching Module Provider](#). This provider stores cached data in the memory of your Medusa application, which is suitable for development and testing purposes.

{/* However, for production, it's recommended to use other Caching Module Providers, such as the [Redis Caching Module Provider](./redis/page.mdx). You can also [Create a Caching Provider](/references/caching/provider). */}

However, for production, it's recommended to use other Caching Module Providers, such as the [Redis Caching Module Provider](./redis/page.mdx). You can also [Create a Caching Provider](#).
...
You can register multiple Caching Module Providers and specify which one to use as the default. Medusa uses the default provider for all caching operations unless you specify a specific provider.

### How is the Default Provider Selected?

If you don't configure the Caching Module in `medusa-config.ts`, Medusa uses the Redis Caching Module Provider as the default provider if you set the `CACHE_REDIS_URL` environment variable:

```bash
CACHE_REDIS_URL=redis://localhost:6379
```

Otherwise, Medusa uses the In-Memory Caching Module Provider as the default.

If you configure the Caching Module, Medusa determines the default provider based on the following scenarios:
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        In-Memory Caching Module Provider.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
