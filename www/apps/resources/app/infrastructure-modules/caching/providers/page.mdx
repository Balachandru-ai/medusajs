...
By default, Medusa uses the [In-Memory Caching Module Provider](./in-memory/page.mdx). This provider stores cached data in the memory of your Medusa application, which is suitable for development and testing purposes.
...
If you don't configure the Caching Module in `medusa-config.ts`, Medusa uses the [In-Memory Caching Module Provider](./in-memory/page.mdx) as the default provider.
