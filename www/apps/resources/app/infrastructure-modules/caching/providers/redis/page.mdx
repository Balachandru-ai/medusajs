...
The Redis Caching Module Provider is registered by default in your Medusa application. You only need to configure the Redis connection URL using the `CACHE_REDIS_URL` environment variable.

For example, in `.env`, add the following variable:

```bash
CACHE_REDIS_URL=redis://localhost:6379
```

The above example assumes you have a Redis server running locally on the default port `6379`. If your Redis server is hosted elsewhere or requires authentication, adjust the URL accordingly.

Your Medusa application will now use the Redis Caching Module Provider as the default caching provider.

### Custom Redis Configuration

If you want to pass custom configurations to the Redis Caching Module Provider or use multiple caching providers, you need to manually register the Redis Caching Module Provider in the `providers` array of the Caching Module.

For example, in `medusa-config.ts`:
...
The Redis Caching Module Provider is used by default in your Medusa application given that you have [configured its URL](#default-redis-configuration) and haven't set up another Caching Module Provider as the [default provider](../page.mdx#default-caching-module-provider).

You can also explicitly specify the Redis Caching Module Provider by using its provider ID `caching-redis` when caching data with Query, the Index Module, or directly with the Caching Module's service.
