...
- Create a Tax Module Provider for Avalara.
    - Test the integration in the checkout process.
- Create transactions in Avalara when an order is placed.
- Sync product variants to Avalara for tax code management.
...
You'll learn how to set these options when you [register the module](#h-add-module-provider-to-medusas-configuration).

### d. Create Connection Loader

A module can have a function, called a [loader](!docs!/learn/fundamentals/modules/loaders), that runs on application startup. Loaders are useful for setting up connections to external services, which you can then use throughout the module.

To create a loader that connects to Avalara, create the file `src/modules/avalara/loaders/connection.ts` with the following content:

export const loaderHighlights = [
  ["15", "", "Validate that the required options are provided."],
  ["23", "client", "Create a new Avatax client with the provided options."],
  ["35", "ping", "Test the connection by calling the ping method."],
  ["44", "register", "Register the Avatax client in the module's container for later use."]
]

```ts title="src/modules/avalara/loaders/connection.ts" highlights={loaderHighlights} collapsibleLines="1-8" expandButtonLabel="Show Imports"
import {
  LoaderOptions,
...
- `container`: The [Module's container](!docs!/learn/fundamentals/modules/container), which is useful for resolving Framework resources or registering custom ones.
- `options`: The options passed to the module when it's registered.

In the loader, you:

1. Throw an error if the required `username`, `password`, and `companyId` options aren't provided.
2. Create a new `Avatax` client using the provided options.
3. Test the connection by calling the `ping` method of the client.
4. If the connection is successful, register the `Avatax` client in the module's container for later use.

### e. Create Avalara Service
...
export const serviceHighlights = [
  ["7", "avatax", "Resolve the Avatax client from the module's container."],
  ["11", "identifier", "The unique identifier of the Avalara Tax Module Provider."],
  ["15", "constructor", "Resolve the Avatax client from the module's container and store the module options."],
]
...
class AvalaraTaxModuleProvider implements ITaxProvider {
  ...
  constructor({ avatax }: InjectedDependencies, options: ModuleOptions) {
    this.avatax = avatax
...
In the constructor, you can resolve resources from the module's container. Here, you resolve the `avatax` client that you registered in the loader. You also receive the module options as a second parameter, which you store for later use.

In addition, you define the `getIdentifier` method required by the `ITaxProvider` interface, which returns the provider's identifier.
...
export const getTaxLinesHighlights = [
  ...
  ["46", "type", "Set the transaction type to SalesOrder to calculate taxes without saving the transaction."]
]
...
2. An object with a required `services` property indicating the Module Provider's services. You also pass the connection loader in the `loaders` property.

### h. Add Module Provider to Medusa's Configuration
...
4. `AVALARA_COMPANY_ID`: The company ID in Avalara for creating product variants for tax code management. To retrieve it from the Avalara dashboard:
...
## Step 5: Sync Product Variants with Avalara

In Avalara, you can manage the items you sell to set classifications, tax codes, exemptions, and other tax-related settings.

In this step, you'll sync Medusa's product variants with Avalara items. This way, you can manage tax codes and other settings for your product variants directly from Avalara.
...
3. Create subscribers that listen to product variant events and trigger the workflows.
...
You'll use these methods in the next sections to build workflows that sync product variants with Avalara items.

### b. Create Avalara Item Workflow

The first workflow you'll build creates an item for a product variant in Avalara. It has the following steps:

<WorkflowDiagram
  workflow={{
    name: "createVariantItemWorkflow",
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        link: "/references/helper-steps/useQueryGraphStep",
        description: "Retrieve product variant's details.",
      },
      {
        type: "step",
        name: "createItemStep",
        description: "Create Avalara item for product variant.",
        depth: 2
      },
      {
        type: "workflow",
        name: "updateProductVariantsWorkflow",
        description: "Save Avalara item ID in product variant metadata.",
...
#### Create Variant Item Workflow

You can now create the workflow. Create the file `src/workflows/create-variant-item.ts` with the following content:

export const createVariantItemWorkflowHighlights = [
  ["12", "variants", "Retrieve the product variant's details."],
  ["28", "stepInput", "Prepare the input for the createItemStep."],
  ["41", "createItemStep", "Create the item in Avalara."],
  ["43", "updateProductVariantsWorkflow", "Save the created item's ID in the product variant's metadata."]
]

```ts title="src/workflows/create-variant-item.ts" highlights={createVariantItemWorkflowHighlights}
...
The workflow receives the product variant ID as input.

In the workflow, you:

1. Retrieve the product variant's details using the `useQueryGraphStep`.
2. Prepare the input for the `createItemStep` using the `transform` function.
3. Call the `createItemStep` to create the item in Avalara.
4. Use the `updateProductVariantsWorkflow` to save the created item's ID in the product variant's metadata. This ID is useful when you want to update or delete the item later.

You return the created item as the workflow's output.

### c. Create Product Variant Created Subscriber

Next, you'll create a subscriber that listens to the `product-variant.created` event and executes the `createVariantItemWorkflow`.

Create the file `src/subscribers/variant-created.ts` with the following content:

```ts title="src/subscribers/variant-created.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
You create a subscriber similar to the one you created for the `order.placed` event. This time, it listens to the `product-variant.created` event and triggers the `createVariantItemWorkflow` with the product variant ID.

### d. Test Product Variant Creation with Avalara Item

To test the product variant creation with Avalara item creation, make sure the Medusa server is running.

Then:

1. Open the Medusa Admin dashboard at `http://localhost:9000/admin`.
2. Go to "Products," and either create a new product or create a variant for an existing product.
3. After creating the product variant, go to your Avalara dashboard.
4. From the sidebar, click on "Settings" → "What you sell and buy."

This will open the Items page, where you can see the variant(s) you created as items in Avalara. You can click on an item to view it, add a classification, and more.

![The list of items in the Avalara dashboard showing the item created for the product variant](https://res.cloudinary.com/dza7lstvk/image/upload/v1761123470/Medusa%20Resources/CleanShot_2025-10-22_at_11.57.33_2x_jbb4km.png)

### e. Update Avalara Item Workflow

Next, you'll create a workflow that updates a product variant's item in Avalara. The workflow has the following steps:

<WorkflowDiagram
  workflow={{
    name: "updateVariantItemWorkflow",
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
      ...
      {
        type: "when",
        condition: "variants.length > 0 && !variants[0].metadata?.avalara_item_id",
        steps: [
          {
            type: "workflow",
            name: "createVariantItemWorkflow",
            description: "Create an item for the variant.",
            depth: 1
          }
        ],
        depth: 2
        ...
        steps: [
          {
            type: "step",
            name: "updateItemStep",
            description: "Update Avalara item for product variant.",
            depth: 2
          }
        ],
        depth: 3
...
Medusa provides the first step out of the box, and you have already created the `createVariantItemWorkflow`. You only need to create the `updateItemStep`.
...
export const updateItemStepHighlights = [
  ["22", "getItem", "Retrieve the original item before updating."],
  ["25", "updateItem", "Update the item in Avalara using the provider's updateItem method."],
  ["42", "updateItem", "Revert the updates by restoring the original values if an error occurs during the workflow's execution."]
]
...
#### Update Variant Item Workflow

You can now create the workflow. Create the file `src/workflows/update-variant-item.ts` with the following content:

export const updateVariantItemWorkflowHighlights = [
  ["13", "variants", "Retrieve the product variant's details."],
  ["30", "when", "Check whether variant does not have an Avalara item ID in its metadata."],
  ["34", "createVariantItemWorkflow", "Create the item in Avalara if it doesn't exist."],
  ["41", "when", "Check whether variant has an Avalara item ID in its metadata."],
  ["45", "transform", "Prepare the input for the updateItemStep."],
  ["55", "updateItemStep", "Update the item in Avalara if it exists."],
...
1. Retrieve the product variant's details using the `useQueryGraphStep`.
2. Use a `when` condition to check whether the variant has an Avalara item ID in its metadata.
    - If it doesn't, you call the `createVariantItemWorkflow` to create the item in Avalara.
...
### f. Create Product Variant Updated Subscriber

Next, you'll create a subscriber that listens to the `product-variant.updated` event and executes the `updateVariantItemWorkflow`.

Create the file `src/subscribers/variant-updated.ts` with the following content:

```ts title="src/subscribers/variant-updated.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
You create a subscriber similar to the one you created for the `product-variant.created` event. This time, it listens to the `product-variant.updated` event and triggers the `updateVariantItemWorkflow` with the product variant ID.

### g. Test Product Variant Update with Avalara Item

To test the product variant update with Avalara item update, make sure the Medusa server is running.

Then:

1. Open the Medusa Admin dashboard at `http://localhost:9000/admin`.
2. Go to "Products," and edit an existing product variant. For example, you can edit its name.
3. After updating the product variant, go to your Avalara dashboard.
4. From the sidebar, click on "Settings" → "What you sell and buy."
    - If the product variant already had an item in Avalara, click on it to view its details and confirm that the changes were applied.
    - If the product variant didn't have an item in Avalara, you should see a new item created for it.

### h. Delete Avalara Item Workflow

The last workflow you'll build deletes a product variant's item from Avalara. The workflow has the following steps:

<WorkflowDiagram
  workflow={{
    name: "deleteVariantItemWorkflow",
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the variant's details.",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1,
      },
      {
        ...
        steps: [
          {
            type: "step",
            name: "deleteItemStep",
            description: "Delete the variant's item from Avalara.",
            depth: 1
          }
        ],
        depth: 2
...
#### Delete Variant Item Workflow

You can now create the workflow. Create the file `src/workflows/delete-variant-item.ts` with the following content:

export const deleteVariantItemWorkflowHighlights = [
  ["12", "variants", "Retrieve the product variant's details."],
  ["27", "when", "Check whether variant has an Avalara item ID in its metadata."],
  ["31", "deleteItemStep", "Delete the item in Avalara if it exists."]
]

```ts title="src/workflows/delete-variant-item.ts" highlights={deleteVariantItemWorkflowHighlights}
...
1. Retrieve the product variant's details using the `useQueryGraphStep`, including deleted variants.
2. Use a `when` condition to check whether the variant has an Avalara item ID in its metadata.
    - If it does, you call the `deleteItemStep` to delete the item in Avalara.

### g. Create Product Variant Deleted Subscriber

Finally, you'll execute the workflow when a product variant is deleted. You need to listen to two events:

1. `product-variant.deleted`: Emitted when a product variant is deleted.
2. `product.deleted`: Emitted when a product is deleted. In this case, you need to delete the items for all variants of the deleted product.

#### Product Variant Deleted Subscriber

To handle a single variant deletion, create the file `src/subscribers/variant-deleted.ts` with the following content:

```ts title="src/subscribers/variant-deleted.ts"
import { SubscriberArgs, type SubscriberConfig } from "@medusajs/framework"
...
You create a subscriber similar to the ones you created before. This time, the subscriber listens to the `product-variant.deleted` event and triggers the `deleteVariantItemWorkflow` with the product variant ID.

#### Product Deleted Subscriber

To handle the deletion of all variants of a deleted product, create the file `src/subscribers/product-deleted.ts` with the following content:
...
You create a subscriber that listens to the `product.deleted` event.

In the subscriber, you:

1. Retrieve the deleted product's variants using Query. Notice that you pass the `withDeleted: true` option to include deleted products and their variants.
2. Loop through the variants and call the `deleteVariantItemWorkflow` for each variant to delete its item from Avalara.

### h. Test Product Variant Deletion with Avalara Item

To test the product variant deletion with Avalara item deletion, make sure the Medusa server is running.

Then:

1. Open the Medusa Admin dashboard at `http://localhost:9000/admin`.
2. Go to "Products," and either delete a single product variant or delete an entire product.
3. After deleting the product variant or product, go to your Avalara dashboard.
4. From the sidebar, click on "Settings" → "What you sell and buy."
    - If you deleted a single product variant, confirm that its corresponding item was removed from Avalara.
    - If you deleted an entire product, confirm that all its variants' corresponding items were removed from Avalara.

---

## Next Steps

You've successfully integrated Avalara with Medusa to handle tax calculations during checkout, create transactions when orders are placed, and sync product variants with Avalara items.
