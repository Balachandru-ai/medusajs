...
npm install payload @payloadcms/next @payloadcms/richtext-lexical sharp graphql @payloadcms/db-postgres
```
...
Next, you'll add hooks that are necessary to sync product data between Medusa and Payload.

#### Initialize Medusa JS SDK

Before adding the hooks, you'll need to create a [JS SDK](../../../js-sdk/page.mdx) instance that allows you to authenticate as an admin user in Medusa to perform product operations.

Create the file `src/lib/payload-sdk.ts` with the following content:

```ts title="src/lib/payload-sdk.ts" badgeLabel="Storefront" badgeColor="blue"
...
You configure the JS SDK to connect to Medusa and use an API key for authentication.

To retrieve the API key from the Medusa Admin:

1. Start the Medusa server with the following command:

```bash npm2yarn badgeLabel="Medusa application" badgeColor="green"
npm run dev
```

2. Open the Medusa Admin dashboard in your browser at `http://localhost:9000/app` and log in with the user you created earlier.
3. Go to Settings -> API Keys.
4. Create an API key and copy it.

Then, add the following environment variables to your Next.js Starter Storefront's `.env.local` file:

```shell title=".env.local" badgeLabel="Storefront" badgeColor="blue"
MEDUSA_PAYLOAD_API_KEY=your_api_key_here
```

<Note>

A Medusa admin user can have one API key only. Learn more in the [API Keys](!user-guide!/settings/developer/secret-api-keys) user guide.

</Note>

#### Add Before Change Hook
...
#### Add After Change Hook

Next, you'll add an `afterChange` hook to the `Products` collection that will sync product data to Medusa when a product is updated in Payload.

In `src/collections/Products.ts`, add an `afterChange` property to the `hooks` configuration:
...
This hook is triggered after a product is updated in Payload. In the hook, you:

1. Validate that the operation is an update and it's not coming from Medusa. This avoids an infinite loop of updates between Payload and Medusa.
2. Prepare the options and variants data to match Medusa's expected format.
3. Use the Medusa JS SDK to update the product in Medusa with the new data.
...
If you click on a product, you can edit its details, such as its title or description. Once you save the changes, these changes will be reflected in Medusa as well since you added an `afterChange` hook to the `Products` collection in Payload.
...
- [product.updated](#a-handle-product-updates): Update the product in Payload when a product is updated in Medusa.
- [product.deleted](#b-handle-product-deletions): Delete the product in Payload when a product is deleted in Medusa.
- [product-variant.created](#c-handle-product-variant-creation): Add a product variant to a product in Payload when a product variant is created in Medusa.
- [product-variant.updated](#d-handle-product-variant-updates): Update a product's variant in Payload when a product variant is updated in Medusa.
- [product-variant.deleted](#e-handle-product-variant-deletions): Remove a product's variant in Payload when a product variant is deleted in Medusa.
- [product-option.created](#f-handle-product-option-creation): Add a product option to a product in Payload when a product option is created in Medusa.
- [product-option.updated](#g-handle-product-option-updates): Update a product's option in Payload when a product option is updated in Medusa.
- [product-option.deleted](#h-handle-product-option-deletions): Remove a product's option in Payload when a product option is deleted in Medusa.

### a. Handle Product Updates

To handle the `product.updated` event, you'll first create a workflow that updates the product in Payload, then create a subscriber that executes the workflow when the event is emitted.

The workflow will have the following steps:

<WorkflowDiagram
  workflow={{
    name: "updatePayloadProductsWorkflow",
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the product from Medusa",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1
      },
      {
        ...
        steps: [
          {
            ...
            description: "Update the product in Payload",
            depth: 1
          }
        ],
        depth: 2
...
You only need to create the `updatePayloadItemsStep`. So, create the file `src/workflows/steps/update-payload-items.ts` with the following content:
...
This step accepts an array of items to update in Payload and the collection slug.

In the step, you retrieve the existing items in Payload using the `find` method of the Payload Module's service. You pass these items to the compensation function so that you can undo the updates if an error occurs in the workflow.

Then, you update the items in Payload using the `update` method of the Payload Module's service and return the updated items.

#### Update Payload Products Workflow 

Next, to create the workflow, create the file `src/workflows/update-payload-products.ts` with the following content:
...
This workflow receives the IDs of the products to update in Payload.

In the workflow, you:

1. Retrieve the product details in Medusa using the `useQueryGraphStep`. You include the `payload_product.*` field to retrieve the linked product data from Payload.
2. Prepare the data to update in Payload using the `transform` function.
    - You don't update variants or options since you will handle those when their respective events are emitted.
3. Update the products in Payload using the `updatePayloadItemsStep` step if there are any items to update.
    - To perform condition checks in a workflow, you must use the `when` function. Learn more in the [Conditions in Workflows](!docs!/learn/fundamentals/workflows/conditions) documentation.
4. Return the updated items from the workflow.

#### Product Updated Subscriber

Finally, to create the subscriber that executes the workflow, create the file `src/subscribers/product-updated.ts` with the following content:
...
Then, open the Medusa Admin at `localhost:9000/app` and open a product's details page. Update the product title or description and save the changes.

If you check the product in Payload, you should see that the product's title and description have been updated there as well.

### b. Handle Product Deletions
...
### c. Handle Product Variant Creation
...
### d. Handle Product Variant Updates
...
1. Retrieve the product variant details from Medusa using the `useQueryGraphStep`, including the linked product data from Payload.
2. Prepare the data to update the product in Payload by finding and updating the existing variant in the variants array.
3. Update the product in Payload using the `updatePayloadItemsStep` if there are any items to update.
4. Return the updated items from the workflow.
...
If you check the product in Payload, you should see that the variant's details have been updated in the product's variants array.

### e. Handle Product Variant Deletions
...
### f. Handle Product Option Creation
...
### g. Handle Product Option Updates

To handle the `product-option.updated` event, you'll create a workflow that updates the option in the corresponding product in Payload.

The workflow will have the following steps:

<WorkflowDiagram
  workflow={{
    name: "updatePayloadProductOptionsWorkflow",
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
      ...
      },
      {
        ...
        steps: [{
          type: "step",
          name: "updatePayloadItemsStep",
          description: "Update the option in the product in Payload",
          depth: 1
        }],
        depth: 2,
      }
...
#### Update Payload Product Options Workflow
...
This workflow receives the IDs of the product options to update in Payload.

In the workflow, you:

1. Retrieve the product option details from Medusa using the `useQueryGraphStep`, including the linked product data from Payload.
2. Prepare the data to update the product in Payload by finding and updating the existing option in the options array.
3. Update the product in Payload using the `updatePayloadItemsStep` if there are any items to update.
4. Return the updated items from the workflow.

#### Product Option Updated Subscriber

Finally, you'll create the subscriber that executes the workflow when the `product-option.updated` event is emitted.
...
Then, open the Medusa Admin at `localhost:9000/app` and open a product's details page. Edit an existing option's title and save the changes.

If you check the product in Payload, you should see that the option's details have been updated in the product's options array.

### h. Handle Product Option Deletions
