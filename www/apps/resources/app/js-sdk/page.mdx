...
If you support [localization](../storefront-development/localization/page.mdx) in your storefront, you can set the locale to retrieve product information and other resources based on the customer's preferred language.

Set the locale using the JS SDK's `setLocale` method. It accepts the locale string in the [IETF BCP 47 standard](https://gist.github.com/typpo/b2b828a35e683b9bf8db91b5404f1bd1).

```ts
sdk.setLocale("fr-FR")
```

All subsequent requests sent using the JS SDK will include the specified locale as a query parameter.
