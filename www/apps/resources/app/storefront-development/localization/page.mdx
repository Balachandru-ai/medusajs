...
The JS SDK will automatically include the `Content-Language` header in API requests. Also, if your application supports `localStorage`, the method sets the locale in the `medusa_locale` key of the `localStorage`.
...
When you fetch the products, their content, such as title and description, is in French and falls back to the store's default locale if a translation isn't available.
