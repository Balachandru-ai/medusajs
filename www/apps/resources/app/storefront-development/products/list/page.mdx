...
By default, the product content like title and description are returned in the store's default locale, falling back to the product's original content if no translation is available.
...
- Set the `Content-Language` header in the API request to the [List Products API route](!api!/store#products_getproducts).
...
If translations aren't available for the selected locale, or no locale is selected, the default content of the product is returned.
