...
4. In the side window that opens, you can edit the store's name, default currency, default region, default sales channel, default location, and default locale. These defaults are used in the storefront if no other values are provided.
...
Locales are available from [Medusa v2.13.0](https://github.com/medusajs/medusa/releases/tag/v2.13.0). If you don't see the locale settings in your Medusa Admin, request your technical team to [upgrade your Medusa application](!docs!/learn/update).

</Note>

Locales define the languages that your store supports for displaying content to customers. You can manage the store's available locales in the store settings. Then, you can translate content like product titles and descriptions into these locales from the [Translations](#) settings.
...
You can now add translations for the added locales in the [Translations](#) settings.

### Set Default Locale

The default locale is used when no specific locale is selected by the customer. Product and content will be displayed in this locale by default.

To set the default locale for your store:

1. Go to Settings → Store.
2. Click the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> icon in the "Store" section's header.
3. Choose Edit from the dropdown.
4. In the side window that opens, select the desired default locale from the "Default Locale" dropdown.
5. Once you're done, click the Save button.
